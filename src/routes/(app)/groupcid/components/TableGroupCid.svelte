...
<div class="table-custom">
	<div class="table-responsive">
		<table class="table table-sm">
			<thead>
				<tr>
					<th class="fw-semibold" style="width: 25%;">{$_("groupcid.group_cid_name")}</th>
					<th class="fw-semibold">{$_("groupcid.tel_campaign_id")}</th>
					<!-- <th class="fw-semibold">{$_("groupcid.tenant_id")}</th> -->
					<th class="fw-semibold">{$_("groupcid.network")}</th>
					<th class="fw-semibold">{$_("groupcid.ref_carrier_uuid")}</th>
					<th class="fw-semibold" style="width: 5%;">{$_("groupcid.order")}</th>
				...
				{#if data?.length > 0}
					{#each paginatedItems as row, i}
						<tr>
							...
							<td><strong>{row.tel_campaign_id}</strong></td>
							<!-- <td><strong>{row.tenant_id}</strong></td> -->
							<td><strong>{row.network}</strong></td>
							<td><strong>{row.ref_carrier_uuid}</strong></td>
							<td><strong>{row.order}</strong></td>
							<td class="btn-group" role="group">
								<div class="d-flex justify-content-center">
										<a class="btn btn-icon btn-soft btn-outline-primary btnDetail" href={`/groupcid/${row.id}`}>
