...
<div class="table-custom">
	<div class="table-responsive">
		<table class="table table-sm">
			<thead>
				<tr>
					<th class="fw-semibold" style="width: 25%;">{$_("server.server_name")}</th>
					<th class="fw-semibold">{$_("server.server_api")}</th>
					<th class="fw-semibold">{$_("server.server_ip")}</th>
					<!-- <th class="fw-semibold">{$_("server.tenant_id")}</th> -->
				...
				{#if data?.length > 0}
					{#each paginatedItems as row, i}
						<tr>
							...
							<td><strong>{row.server_api}</strong></td>
							<td><strong>{row.server_ip}</strong></td>
							<!-- <td><strong>{row.tenant_id}</strong></td> -->
							<td class="btn-group" role="group">
								<div class="d-flex justify-content-center">
										<a class="btn btn-icon btn-soft btn-outline-primary btnDetail" href={`/server/${row.id}`}>
